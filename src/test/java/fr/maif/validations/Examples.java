...
public class Examples {
    ...
    public static class VikingValidation {
        ...
        public static final String WEBSITE_URL_PATTERN = "^(http:\\/\\/www\\.|https:\\/\\/www\\.|http:\\/\\/|https:\\/\\/)?[a-z0-9_]+([\\-\\.]{1}[a-z0-9_]+)*\\.[a-z]{2,9}(:[0-9]{1,5})?(\\/.*)?$";
    ...
    // Fake call to api
    public Future<Boolean> userExists(String name) {
        return Future(true);
    }


    public Future<Rule<String>> validateUserExists(String name) {
        return userExists(name).map(exists -> {
            if (exists) {
                return Rule.valid();
            } else {
                return Rule.invalid("User "+name+" not found");
            }
        });
    }
